			...
			union iwreq_data *wdata, char *extra)
{
	...
	{
		DBG_871X(" %s fail =>(padapter->bup == false )\n",__FUNCTION__);
		return -ENETDOWN;
	}

	if (RTW_CANNOT_RUN(padapter)) {
		DBG_871X("%s fail =>(bSurpriseRemoved == true) || ( bDriverStopped == true)\n", __func__);
		return -ENETDOWN;
       }
			...
			union iwreq_data *wdata, char *extra)
{
	...
	if (RTW_CANNOT_RUN(padapter)) {
		DBG_871X("%s fail =>(padapter->bSurpriseRemoved == true) || ( padapter->bDriverStopped == true)\n", __func__);
		return -ENETDOWN;
       }
				...
				union iwreq_data *wrqu, char *extra)
{
	...
	if (padapter->tdlsinfo.ch_switch_prohibited == true)
	{
		DBG_871X("Can't do TDLS channel switch since ch_switch_prohibited = true\n");
		return ret;
	}
